...
---- #                                                                       #
---- # Battery internal resistance widget                                    #
---- # 04/02/2020 Phaedra Degreef                                            #
---- # Based on the Battcheck widget, modified to calculate and display      #
---- # the (estimated) internal resistance of the battery                    #
---- # Calculation is done based on the measured voltage (Voc) when the      #
---- # current drawn is almost 0.  When the current then first exceeds a     #
---- # given threshold, the internal resistance can then be calculated       #
---- # by measuring the battery voltage again under load.                    #
---- #                                                                       #
---- #########################################################################

local shadowed  = 0
local initCurr = false
local consThreshold = 20
local currentThreshold = 10.0
local thresholdPassed = false
local Voc = 0
local VocRef = 0
local internalResistance = 0
local current = 0
local cons = 0
local nrCells = 3

local options = {
  { "NrOfCells", VALUE, 4,1,6 },
  { "Sensor", SOURCE, 1 },
  { "SensorVOC", SOURCE, 1 },
  { "SensorCurrent", SOURCE, 1 },
  { "SensorCons", SOURCE, 1 }
--  { "Color", COLOR, WHITE },
--  { "Shadow", BOOL, 0 }
}

local function getInternalResistance(zone)
  Voc = getValue(zone.options.SensorVOC)
  current = getValue(zone.options.SensorCurrent)
  cons = getValue(zone.options.SensorCons)
  nrCells = getValue(zone.options.NrOfCells)

  Voc = getValue(zone.options.SensorVOC)
  if initCurr then
    -- calculate internal resistance as soon as the consumption exceeds 200mAh
	if not thresholdPassed then
	  if cons > consThreshold and current > currentThreshold then
		-- calculate internal resistance
		internalResistance = (VocRef-Voc)/current
		if internalResistance*1000 > 20 then -- only accept realistic values
			thresholdPassed = true
		end
	  end
	end
  else
    -- as long as the measured current hasn't exceeded 1A, our calculation isn't yet initialized
	-- We measure the maximum Voc (open-circuit voltage)
	if Voc > VocRef then
	  VocRef = Voc
	end
	if current >= 1.0 then
...
local function checkNILNum(x,y)
  if x~=nil then 
    return x
  else
   return y 
...
--- This function return the percentage remaining in a Lipo battery
local function getBattPercent(cellValue, nrOfCells)
  --## Data gathered from commercial lipo sensors
  local myArrayPercentList =
{{3, 0}, {3.093, 1}, {3.196, 2}, {3.301, 3}, {3.401, 4}, {3.477, 5}, {3.544, 6}, {3.601, 7}, {3.637, 8}, {3.664, 9}, {3.679, 10}, {3.683, 11}, {3.689, 12}, {3.692, 13}, {3.705, 14}, {3.71, 15}, {3.713, 16}, {3.715, 17}, {3.72, 18}, {3.731, 19}, {3.735, 20}, {3.744, 21}, {3.753, 22}, {3.756, 23}, {3.758, 24}, {3.762, 25}, {3.767, 26}, {3.774, 27}, {3.78, 28}, {3.783, 29}, {3.786, 30}, {3.789, 31}, {3.794, 32}, {3.797, 33}, {3.8, 34}, {3.802, 35}, {3.805, 36}, {3.808, 37}, {3.811, 38}, {3.815, 39}, {3.818, 40}, {3.822, 41}, {3.825, 42}, {3.829, 43}, {3.833, 44}, {3.836, 45}, {3.84, 46}, {3.843, 47}, {3.847, 48}, {3.85, 49}, {3.854, 50}, {3.857, 51}, {3.86, 52}, {3.863, 53}, {3.866, 54}, {3.87, 55}, {3.874, 56}, {3.879, 57}, {3.888, 58}, {3.893, 59}, {3.897, 60}, {3.902, 61}, {3.906, 62}, {3.911, 63}, {3.918, 64}, {3.923, 65}, {3.928, 66}, {3.939, 67}, {3.943, 68}, {3.949, 69}, {3.955, 70}, {3.961, 71}, {3.968, 72}, {3.974, 73}, {3.981, 74}, {3.987, 75}, {3.994, 76}, {4.001, 77}, {4.007, 78}, {4.014, 79}, {4.021, 80}, {4.029, 81}, {4.036, 82}, {4.044, 83}, {4.052, 84}, {4.062, 85}, {4.074, 86}, {4.085, 87}, {4.095, 88}, {4.105, 89}, {4.111, 90}, {4.116, 91}, {4.12, 92}, {4.125, 93}, {4.129, 94}, {4.135, 95}, {4.145, 96}, {4.176, 97}, {4.179, 98}, {4.193, 99}, {4.2, 100}}
  nrCells = checkNILNum(NrOfCells,4)

  for i, v in ipairs( myArrayPercentList ) do
    if v[ 1 ]*nrCells >= cellValue then
      result =  v[ 2 ]
      break
     end
  end
...
-- Display only percent and resistance
local function zoneTiny(zone)
  
  getInternalResistance(zone) -- get the values of voltage, current and resistance
  local myString = string.format("%dmOh", internalResistance*1000) -- resistance is in mOhm
  local percent = getBattPercent(Voc, zone.options.nrOfCells)
--  local percent = getBattPercent(Voc, 4)

  local tmpmyString = string.format("%2.1f V", Voc)
  local tmpmyString2 = string.format("%2.1f V", VocRef)

    lcd.drawText(zone.zone.x + zone.zone.w, zone.zone.y, percent.."%", RIGHT + SMLSIZE + CUSTOM_COLOR)
--    lcd.drawText(zone.zone.x + zone.zone.w, zone.zone.y, tmpmyString, RIGHT + SMLSIZE + CUSTOM_COLOR)
    if internalResistance > 0 then
      lcd.drawText(zone.zone.x + zone.zone.w, zone.zone.y + 15, myString, RIGHT + SMLSIZE + CUSTOM_COLOR)
	else
      lcd.drawText(zone.zone.x + zone.zone.w, zone.zone.y + 15, "-----", RIGHT + SMLSIZE + CUSTOM_COLOR)
	end
    -- draw batt
    ...
    lcd.drawFilledRectangle(zone.zone.x+2, zone.zone.y + 6 + 25 - rect_h , 12, rect_h, CUSTOM_COLOR)
--   end
end
...
--  local myString = string.format("%2.1fV", getCellSum(mySensor)).." ("..getCellCount(mySensor).."S)"
    local myString = string.format("%dmO", internalResistance*1000) -- resistance is in mOhm
    local percent = getCellPercent(Voc)
    ...
    lcd.drawText(zone.zone.x, zone.zone.y+10, "No sensor data", LEFT + SMLSIZE + INVERS + CUSTOM_COLOR + shadowed)
...
local function zoneMedium(zone)
  ...
  getInternalResistance(zone) -- get the values of voltage, current and resistance
  local myString = string.format("%dmOhm", internalResistance*1000) -- resistance is in mOhm
  local percent = getBattPercent(Voc, zone.options.nrOfCells)
--  local percent = getBattPercent(Voc, 4)
...
--  lcd.setColor(CUSTOM_COLOR, zone.options.Color)
  if Voc > 0 then
    local vocString = string.format("%2.1f V", Voc)
    local vocRefString = string.format("%2.1f V", VocRef)
    ...
    lcd.setColor(CUSTOM_COLOR, WHITE)
    lcd.drawText(zone.zone.x + myBatt.x + 1 , zone.zone.y + 35, myString, LEFT + MIDSIZE + CUSTOM_COLOR + shadowed)
    lcd.drawText(zone.zone.x + zone.zone.w, zone.zone.y + 35, string.format("%2.1fA", current), RIGHT + MIDSIZE + CUSTOM_COLOR + shadowed)
    lcd.drawText(zone.zone.x + zone.zone.w, zone.zone.y + 57, string.format("consumption %d mAh", cons), RIGHT + SMLSIZE + CUSTOM_COLOR + shadowed)
	  
  else
    lcd.drawText(zone.zone.x, zone.zone.y+35, "No battery sensor data", LEFT + SMLSIZE + INVERS + CUSTOM_COLOR)
  end
  -- draws bat
  lcd.setColor(CUSTOM_COLOR, WHITE)
  lcd.drawRectangle(zone.zone.x + myBatt.x , zone.zone.y + myBatt.y, myBatt.w, myBatt.h, CUSTOM_COLOR, 2)
  ...
  lcd.drawText(zone.zone.x + myBatt.x + 5 , zone.zone.y + myBatt.y + 5, string.format("%2.1fV", Voc), LEFT + MIDSIZE + CUSTOM_COLOR + shadowed)
...
--    local percent = getCellPercent(getCellAvg(mySensor))
    ...
    lcd.drawText(zone.zone.x+zone.zone.w, zone.zone.y+44, string.format("%2.1fV", Voc), RIGHT + MIDSIZE + CUSTOM_COLOR + shadowed)
    lcd.drawText(zone.zone.x+zone.zone.w, zone.zone.y+65, zone.options.nrOfCells.."S", RIGHT + MIDSIZE + CUSTOM_COLOR + shadowed)
    -- fils batt
    lcd.setColor(CUSTOM_COLOR, getPercentColor(percent))
...
--    end
    ...
    lcd.drawText(zone.zone.x+5, zone.zone.y, "No battery sensor data", LEFT + SMLSIZE + INVERS + CUSTOM_COLOR + shadowed)
  end
  -- draws bat
  lcd.setColor(CUSTOM_COLOR, WHITE)
...
return { name="BattIntRes", options=options, create=create, update=update, background=background, refresh=refresh }
